...
function dingdan(id){
	window.location.href="/information/order/OrderExerciser/"+id;
}
function yuyue(id){
    window.location.href="/information/yuyue/YuyueExerciser/"+id;
}
function dizhi(id){
	window.location.href="/information/address/"+id;
}
function yaoqing(id){
window.location.href="/information/yaoqing/"+id;
}

function jifen(id) {
    window.location.href="/information/scores/"+id;
}

function shenshen(id){
    layer.open({
        type : 2,
        title : '教练审核',
        maxmin : true,
        shadeClose : false, // 点击遮罩关闭层
        area : [ '800px', '520px' ],
        content : prefix + '/shenshen/'+id // iframe的url
    });
}
